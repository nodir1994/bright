...
document.addEventListener("scroll", (e) => {
  if (window.scrollY > section.offsetTop && (window.scrollY - section.offsetTop)<window.innerHeight/2) {
    $(".car_block img").css({
      transform: "translateX(" + 3*(window.scrollY - section.offsetTop) + "px)",
    });
  }
});
...
const cursor = document.getElementById("bubble-cursor"),
    ...
    width = 36,
    idleTimeout = 150;
let timeoutID=200, hoverButton, hoverTL, lastFrame = 0,
...
class HoverButton {
    ...
    hoverInAnim() {
        this.hovered || (this.hovered = !0, this.animatingHover = !0, this.forceOut = !1, TweenMax.fromTo(this.bg, this.timing, {
                x: "-112%"
            }, {
                x: "-12%",
                ease: Power3.easeOut,
                onComplete: () => {
                    this.animatingHover = !1, this.forceOut && (this.foceOut = !1, this.hoverOutAnim())
                }
            }))
    }
    onMouseLeave() {this.animatingHover ? this.forceOut = !0 : this.hoverOutAnim()}
    hoverOutAnim() {
        this.hovered = !1,
        TweenMax.to(this.bg, this.timing, {x: "100%",ease: Power3.easeOut,onComplete: () => {}})
    }
}
class Dot {
    constructor(e = 0) {
        this.index = e, this.anglespeed = .05, this.x = 0, this.y = 0, this.scale = 1 - .05 * e, 
          this.range = width / 2 - width / 2 * this.scale + 2, this.limit = .75 * width * this.scale, 
